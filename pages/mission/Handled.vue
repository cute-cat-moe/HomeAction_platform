        ...
        <el-table
          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              {{ scope.row.start_time * 1000 | date('YYYY-MM-DD HH:mm') }}
            </template>
          </el-table-column>
          <el-table-column
            prop="end_time"
            ...
            width="width">
            <template slot-scope="scope">
              {{ scope.row.end_time * 1000 | date('YYYY-MM-DD HH:mm') }}
            </template>
          </el-table-column>
          <el-table-column
            prop="finded"
            ...
            width="width">
            <template slot-scope="scope">
              <finded-tag :finded="scope.row.finded"></finded-tag>
            </template>
          </el-table-column>
          <el-table-column
            prop="t_level"
...
export default defineComponent ({
  components: {
    ...
    setup(props, ctx) {
        ...
        ctx.root.$http.get('admin/task_center/task_finish').then(res => {
          taskHandledList.value = res.data.data
        }).catch(err => {
          console.error(err)
          // ctx.root.$message.error(err)
        })
        ...
        const handleClick = (val) => {
          ctx.root.$router.push(ctx.root.$getMissionHandledDetailRoute(val.tid))
        }
